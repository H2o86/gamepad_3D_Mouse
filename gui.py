...
class ApplicationGUI(tk.Tk):
    ...
    def create_widgets(self):
        ...
        lbl_title = ttk.Label(
            header,
            text="Flydigi Dune Fox - SolidWorks 3D SpaceMouse Control Center",
        ...
        lbl_info = ttk.Label(
            tab_keys,
            text="Chọn chức năng phím tắt SolidWorks 2019 hoặc nhập phím tùy chỉnh (VD: Ctrl+7, Shift+E, F5):",
        ...
        for idx, (btn_id, btn_name) in enumerate(BUTTON_DEFS):
            row_frame = ttk.Frame(scrollable_frame, padding=4)
            row_frame.pack(fill="x", expand=True, pady=2)

            lbl_btn = ttk.Label(row_frame, text=btn_name, width=22, font=("Segoe UI", 10, "bold"))
            ...
            entry = ttk.Entry(row_frame, textvariable=entry_var, width=20, font=("Consolas", 10))
        ...
        # Tab 2: Window Filter & Cursor Options
        tab_options = ttk.Frame(notebook, padding=15)
        notebook.add(tab_options, text="⚙️ Cửa Sổ SolidWorks & Con Trỏ Chuột")

        frame_wf = ttk.LabelFrame(tab_options, text=" Chế Độ Kích Hoạt Phần Mềm ", padding=12)
        frame_wf.pack(fill="x", pady=(0, 15))
        ...
        chk_target = ttk.Checkbutton(
            frame_wf,
            text="Chỉ kích hoạt điều khiển khi bật cửa sổ SOLIDWORKS (Ngăn ảnh hưởng ứng dụng khác)",
            variable=self.var_target_only,
        )
        chk_target.pack(anchor="w", pady=5)

        frame_cl = ttk.LabelFrame(tab_options, text=" Chế Độ Con Trỏ Chuột (Cursor Anti-Border Drift) ", padding=12)
        frame_cl.pack(fill="x", pady=(0, 15))
        ...
        chk_lock = ttk.Checkbutton(
            frame_cl,
            text="Chống con trỏ văng viền màn hình (Edge Wrapping - Giúp xoay mô hình 3D mượt 100%, không bị giật)",
            variable=self.var_lock_cursor,
        )
        chk_lock.pack(anchor="w", pady=5)

        lbl_lock_info = ttk.Label(
            frame_cl,
            text="📌 Giúp xoay liên tục không bị gián đoạn lệnh. Khi thả cần gạt, con trỏ tự động trả về vị trí ban đầu.",
            font=("Segoe UI", 9, "italic"),
            foreground="#2563EB",
        )
        lbl_lock_info.pack(anchor="w", pady=(5, 0))

        frame_sens = ttk.LabelFrame(tab_options, text=" Tùy Chỉnh Độ Nhạy ", padding=12)
        ...
        self.slider_pan.pack(fill="x", pady=(0, 5))
        ...
        self.slider_rotate.pack(fill="x", pady=(0, 5))

        ttk.Label(frame_sens, text="Tốc độ Zoom (LT/RT):", font=("Segoe UI", 9, "bold")).pack(anchor="w")
        self.slider_zoom = ttk.Scale(frame_sens, from_=2.0, to_=30.0, value=self.config_data["sensitivity"]["zoom"])
        self.slider_zoom.pack(fill="x", pady=(0, 5))
        ...
        self.slider_deadzone.pack(fill="x", pady=(0, 5))
    ...
    def run_loop(self):
        ...
        while self.running:
            ...
            if state:
                if is_sw_active:
                    status_str = "🟢 Đã kết nối Flydigi Dune Fox | Cửa sổ SolidWorks ACTIVE"
                    status_clr = "#10B981"
                else:
                    status_str = f"🟡 Đã kết nối tay cầm | Cửa sổ đang mở: [{proc_name}] (Bật SolidWorks để điều khiển)"
                    status_clr = "#F59E0B"
                ...
                txt = (
                    f"--- ACTIVE WINDOW & CONTROLLER DIAGNOSTICS ---\n"
                    ...
                    f"Edge Border Wrap    : {'ENABLED (Smooth rotation, no border drift)' if self.navigator.config.get('app_filter', {}).get('lock_cursor_center') else 'DISABLED'}\n"
                    f"----------------------------------------------\n"
                ...
                self.lbl_status.config(text="Trạng thái: 🔴 Không tìm thấy tay cầm (Hãy cắm USB / 2.4G Dongle)", foreground="#EF4444")
                self.txt_monitor.delete("1.0", tk.END)
                self.txt_monitor.insert(tk.END, "Vui lòng cắm tay cầm Flydigi Dune Fox qua cáp USB hoặc Receiver 2.4G...\n")
